          ...
          {items.length ? (
            <ul className="min-w-0 border-t border-border">
              {items.map((item, index) => (
                <li
                  className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)] gap-x-4 border-b border-border py-6 sm:gap-x-6 sm:py-8"
                  key={`${item.title}-${index}`}
                >
                  <span
                    aria-hidden="true"
                    className="pt-1 font-display text-sm font-semibold tabular-nums tracking-[0.12em] text-muted-text"
                  >
                    {String(index + 1).padStart(2, "0")}
                  </span>

                  <div className="min-w-0">
                    <h3 className="break-words font-display text-xl font-semibold tracking-tight text-text sm:text-2xl">
                      {item.title}
                    </h3>
